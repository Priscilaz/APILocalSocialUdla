...
namespace UdlaBlog.WebAPI.Local.Controllers
{
    ...
    [ApiController]
    public class FicaBlogsController : ControllerBase
    {
        private readonly IBlogPostRepository _repository;
        private readonly IWebHostEnvironment _webHostEnvironment;

        public FicaBlogsController(IBlogPostRepository repository, IWebHostEnvironment webHostEnvironment)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<IEnumerable<BlogPostDto>>> GetFicaBlogs()
        {
            var blogPosts = await _repository.GetBySectionAsync("Fica");
            var blogPostsDto = blogPosts.Select(b => ConvertToDto(b));
            return Ok(blogPostsDto);
        }

        [HttpGet("{id}")]
        public async Task<ActionResult<BlogPostDto>> GetFicaBlog(Guid id)
        {
            var blogPost = await _repository.GetByIdAndSectionAsync(id, "Fica");
            if (blogPost == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public async Task<ActionResult<BlogPostDto>> PostFicaBlog([FromForm] BlogPostDto blogPostDto)
        {
            var blogPost = ConvertToEntity(blogPostDto);
            blogPost.Section = "Fica";
            ...
            return CreatedAtAction(nameof(GetFicaBlog), new { id = blogPost.Id }, ConvertToDto(blogPost));
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> PutFicaBlog(Guid id, [FromForm] BlogPostDto blogPostDto)
        {
        ...
        [HttpDelete("{id}")]
        public async Task<IActionResult> DeleteFicaBlog(Guid id)
        {
            var blogPost = await _repository.GetByIdAndSectionAsync(id, "Fica");
            if (blogPost == null)
            {
